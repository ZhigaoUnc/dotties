...
/**
 * 更 UPDATES sub-surface: a terminal-free face for the dingaling update engine. It
 * never touches git itself; it shells out to the python engine at
 * ~/.config/hypr/scripts/dingaling-update.py, which prints one JSON object, and
...
SettingsSurface {
    ...
    readonly property string engine: Quickshell.env("HOME") + "/.config/hypr/scripts/dingaling-update.py"
    ...
    Process {
        ...
        command: ["sh", "-c",
            "d=\"${XDG_STATE_HOME:-$HOME/.local/state}/dingaling\"; mkdir -p \"$d\"; printf '%s' \"$1\" > \"$d/updated\"",
            "sh", body]
    }
