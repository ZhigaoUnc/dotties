...
/**
 * Wallpaper surface: a filmstrip over ~/Pictures/Wallpapers, rendered as one
...
PillSurface {
    ...
    readonly property string searchScript: Quickshell.env("HOME") + "/.config/scripts/dingaling/wallpaper-search.sh"
        ...
        delegate: Item {
            ...
            Rectangle {
                ...
                border.color: {
                    ...
                    if (tile.committing)
                        return Theme.vermLit;
                    if (!tile.remote && tile.modelData && tile.modelData.path === Walls.current)
                        return Theme.vermLit;
                    return Theme.border;
                }
