 ...
 * flags (~/.local/state/dingaling/events.json). The in-memory `events` is the
...
Singleton {
    ...
    readonly property string stateDir: (Quickshell.env("XDG_STATE_HOME") || (Quickshell.env("HOME") + "/.local/state")) + "/dingaling"
