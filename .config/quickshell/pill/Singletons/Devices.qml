...
/**
 * Single owner of screen nightlight (gammastep) and external-monitor brightness
 * (ddcutil) for the mixer. The persisted nightlight percent is the source of
 * truth: loaded and re-applied once at startup so the tint survives a reboot,
 * and every later set both pushes to gammastep and writes back the state file.
...
Singleton {
    ...
    readonly property string stateFile: (Quickshell.env("XDG_STATE_HOME") || (Quickshell.env("HOME") + "/.local/state")) + "/dingaling/gammastep-value"

    /** Nightlight slider 0..100, maps to 6500K..3500K via gammastep. */
    property int nightlightPct: 0

    /** Whether nightlight adjustment is currently applied. */
    ...
    /**
     * Loads the persisted nightlight percent and applies it once, so the saved
     * tint is restored on boot. Singletons init lazily, so a startup caller
     * must reference this for the restore to fire.
     */
    function restore() {
        var raw = nightlightState.text();
        var v = parseInt((raw || "0").trim());
        root.nightlightPct = isNaN(v) ? 0 : v;
        root.nightlightEnabled = root.nightlightPct > 0;
        if (root.nightlightEnabled)
            applyNightlight(root.nightlightPct);
    }

    /**
     * Toggles nightlight on/off. When turning off, kills gammastep to reset.
     */
    function toggleNightlight() {
        ...
        if (root.nightlightEnabled) {
            ...
            Quickshell.execDetached(["sh", "-c", "killall gammastep 2>/dev/null"]);
        }
    }

    /**
     * Sets the nightlight warmth to `pct` percent: pushes it to gammastep and
     * persists it to the state file. Enables nightlight if not already on.
     */
    function setNightlight(pct) {
        root.nightlightPct = Math.round(Math.max(0, Math.min(100, pct)));
        if (!root.nightlightEnabled) {
            root.nightlightEnabled = true;
        }
        applyNightlight(root.nightlightPct);
        saveNightlight(root.nightlightPct);
    }

    function applyNightlight(pct) {
        var temp = 6500 - Math.round(Math.max(0, Math.min(100, pct)) * 30);
        Quickshell.execDetached(["sh", "-c",
            "killall gammastep 2>/dev/null; gammastep -PO " + String(temp)]);
    }

    function saveNightlight(pct) {
        Quickshell.execDetached(["sh", "-c",
