...
PillSurface {
    ...
    readonly property var actions: [
        { key: "lock",     glyph: "lock",     label: "Lock",     confirm: true,  dispatch: "",             argv: [Quickshell.env("HOME") + "/.config/scripts/dingaling/lock"] },
    ...
    readonly property var actionLetters: ({
        lock: Flags.powerKeyLock,
        logout: Flags.powerKeyLogout,
        suspend: Flags.powerKeySuspend,
        reboot: Flags.powerKeyReboot,
        shutdown: Flags.powerKeyShutdown
    })

    readonly property int splitAfter: 2

    property int heldLetterIdx: -1

    function actionIndexByLetter(letter) {
        for (var i = 0; i < root.actions.length; i++)
            if (root.actionLetters[root.actions[i].key] === letter)
                return i;
        return -1;
    }

    function keyAction(letter) {
        var idx = root.actionIndexByLetter(letter);
        if (idx < 0) return false;
        var a = root.actions[idx];
        if (a.confirm) {
            root.heldLetterIdx = idx;
            letterHeat.press();
        } else {
            root.run(a);
        }
    ...
    function keyRelease(letter) {
        var idx = root.actionIndexByLetter(letter);
        if (idx < 0) return;
        if (root.heldLetterIdx === idx) {
            root.heldLetterIdx = -1;
            letterHeat.release();
        }
    }
    ...
    HeatHold {
        id: letterHeat
        onConfirmed: {
            if (root.heldLetterIdx >= 0)
                root.run(root.actions[root.heldLetterIdx]);
            root.heldLetterIdx = -1;
        }
    }
        ...
        Repeater {
            ...
            delegate: Row {
                ...
                Item {
                    ...
                    readonly property real hold: root.heldLetterIdx === cell.index ? letterHeat.hold : heat.hold
    ...
    Text {
        ...
        text: act ? root.actionLetters[act.key].toUpperCase() + " " + act.label : ""
