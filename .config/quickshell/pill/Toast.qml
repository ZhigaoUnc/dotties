...
Item {
    ...
    Timer {
        ...
        running: root.deadline > 0 && root.live && root.notif.urgency !== NotificationUrgency.Critical && !root.dismissing
    ...
    SequentialAnimation {
        id: dismissAnim
        ParallelAnimation {
            NumberAnimation { target: root; property: "scale"; to: 0.85; duration: 120; easing.type: Easing.OutCubic }
            NumberAnimation { target: root; property: "opacity"; to: 0; duration: 150; easing.type: Easing.OutCubic }
        }
        ScriptAction { script: Notifs.removePopup(root.notif) }
    }
