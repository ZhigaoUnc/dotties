...
Item {
    ...
    readonly property string hsCon: "dingalingHotspot"
    readonly property string hsIface: wifiDev ? (wifiDev.name || "wlan0") : "wlan0"
    property string hsName: "dingaling"
    ...
    Item {
        ...
        Column {
            ...
            component CredRow: Item {
                ...
                Text {
                    visible: !cr.editing
                    anchors.right: eyeToggle.visible ? eyeToggle.left : parent.right
                    anchors.rightMargin: 4 * root.s
                    anchors.verticalCenter: parent.verticalCenter
                    text: cr.value.length ? (cr.secret && !cr.showSecret ? "•".repeat(cr.value.length) : cr.value) : "tap to set"
                ...
                GlyphIcon {
                    id: eyeToggle
                    visible: !cr.editing && cr.secret && cr.value.length > 0
                    anchors.right: parent.right
                    anchors.rightMargin: 8 * root.s
                    anchors.verticalCenter: parent.verticalCenter
                    width: 15 * root.s
                    height: 15 * root.s
                    name: cr.showSecret ? "eye-off" : "eye"
                    color: cr.showSecret ? Theme.flameGlow : Theme.iconDim
                    stroke: 1.5
                    ...
                    MouseArea {
                        ...
                        cursorShape: Qt.PointingHandCursor
                        onClicked: cr.showSecret = !cr.showSecret
                    }
                }
