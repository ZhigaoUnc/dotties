...
/**
 * Mixer surface: header with DND / Keep-Awake / Nightlight chips and a row of
 * four vertical ink-faders wired to real hardware (brightness via ddcutil,
 * nightlight via gammastep, volume and mic via Pipewire). Fills the lower body
 * of the pill.
 */
PillSurface {
    id: root
    ...
    readonly property var faders: {
        ...
        out.push(nightlightFader, volFader, micFader);
        return out;
    }
    ...
    property real pendingNightlight: -1
    property int pendingBacklight: -1

    Timer {
        id: nightlightDebounce
        interval: 160
        onTriggered: if (root.pendingNightlight >= 0) {
            Devices.setNightlight(root.pendingNightlight);
            root.pendingNightlight = -1;
        }
    }
    ...
    Item {
        ...
        Row {
            ...
            IconChip {
                glyph: "moon"
                on: Devices.nightlightEnabled
                tipTitle: "Night light"
                tipDesc: "Warm screen colours"
                onToggled: Devices.toggleNightlight()
            }
        }
    }
    ...
    Row {
        ...
        VFader {
            ...
            valueLabel: {
                var temp = 6500 - Math.round(Math.max(0, Math.min(100, Devices.nightlightPct)) * 30);
                return temp + "K";
            }
            onMoved: (v) => Devices.nightlightPct = Math.round(v * 100)
            onCommitted: (v) => { root.pendingNightlight = v * 100; nightlightDebounce.restart(); }
