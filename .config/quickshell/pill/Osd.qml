...
Item {
    ...
    readonly property real osdScale: Flags.osdScale
    ...
    readonly property real desiredW: kind === "workspace" ? Math.max(120 * s, wsIndicator.implicitWidth + 40 * s)
        : (kind === "track" ? 332 * s : (kind === "record" ? 256 * s : (kind === "dnd" ? 270 * s : 248 * s)))
    readonly property real desiredH: kind === "workspace" ? 38 * s / osdScale : (kind === "track" ? 56 * s : 44 * s)
    ...
    Connections {
        target: Flags
        function onDndChanged() { root.flash("dnd"); }
    }
    ...
    Process {
        id: brightMonitor
        command: ["sh", "-c", "dev=$(ls /sys/class/backlight 2>/dev/null | head -n1); [ -n \"$dev\" ] || exit 0; max=$(cat /sys/class/backlight/$dev/max_brightness); last=\"\"; while true; do val=$(cat /sys/class/backlight/$dev/brightness); if [ \"$val\" != \"$last\" ]; then echo \"$(( val * 100 / max ))\"; last=\"$val\"; fi; sleep 0.1; done"]
        running: true
        stdout: SplitParser {
            onRead: (line) => {
            ...
            Rectangle {
                ...
                Rectangle {
                    ...
                    gradient: Gradient {
                        ...
                        GradientStop { position: 0.5; color: "#55d4d4d4" }
    ...
    Item {
        ...
        s: root.s / root.osdScale
        gap: 8 * root.s / root.osdScale
        enabled: false
        }
    }
    ...
    Item {
        ...
        Row {
            ...
            GlyphIcon {
                ...
                height: 17 * root.s
                name: "bell-off"
                color: Flags.dnd ? Theme.vermLit : Theme.iconDim
                stroke: 1.7
            }

            Text {
                text: "Do not disturb" + (Flags.dnd ? " · On" : " · Off")
                color: Flags.dnd ? Theme.cream : Theme.subtle
                font.family: Theme.font
                font.pixelSize: 11.5 * root.s
                font.weight: Font.DemiBold
                elide: Text.ElideRight
    ...
    Item {
        ...
        GlyphIcon {
            ...
            width: 17 * root.s
            height: 17 * root.s
            name: root.source && root.source.audio && root.source.audio.muted ? "mic-off" : "mic"
            color: root.source && root.source.audio && root.source.audio.muted ? Theme.dim : Theme.iconDim
            stroke: 1.7
        }
        ...
        Text {
            ...
            width: 36 * root.s
            horizontalAlignment: Text.AlignRight
            text: root.source && root.source.audio && root.source.audio.muted ? "off"
                : (root.source && root.source.audio ? Math.round(root.source.audio.volume * 100) + "%" : "—")
            color: root.source && root.source.audio && root.source.audio.muted ? Theme.dim : Theme.cream
            font.family: Theme.font
            font.pixelSize: 11 * root.s
            font.weight: Font.DemiBold
            font.features: { "tnum": 1 }
        ...
        Rectangle {
            ...
            Rectangle {
                ...
                anchors.top: parent.top
                anchors.bottom: parent.bottom
                width: parent.width * (root.source && root.source.audio ? Math.max(0, Math.min(1, root.source.audio.volume)) : 0)
                radius: parent.radius
                color: root.source && root.source.audio && root.source.audio.muted ? Theme.vermDim : Theme.vermLit
                Behavior on width { NumberAnimation { duration: Motion.fast } }
                Behavior on color { ColorAnimation { duration: Motion.fast } }
            }
        }
